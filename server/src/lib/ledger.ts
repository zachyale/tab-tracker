...
const UNPRICED_EXPR = sql`
  COALESCE(SUM(CASE
    WHEN kind = 'drink' AND amount_cents IS NULL THEN 1
...
export function quantitiesForUser(accountId: string, userId: string): Record<string, number> {
  const rows = db.all<{ optionId: string; qty: number }>(sql`
    SELECT option_id AS optionId,
           SUM(CASE kind WHEN 'drink' THEN 1 WHEN 'undo' THEN -1 ELSE 0 END) AS qty
    FROM entry
...
/** The most recent drink entry for user+option that hasn't been undone yet. */
export function latestUndoableDrink(
  accountId: string,
  userId: string,
  optionId: string
